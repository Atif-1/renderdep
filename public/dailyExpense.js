...
window.addEventListener('DOMContentLoaded',async ()=>{
	try{
	const page=1;
	const result=await axios.get(`http://43.205.116.221:3000/expense/getExpenses/${page}`,{headers:{'Authorization':token,'rows':localStorage.getItem("rows")}})
		...
		const downloads=await axios.get("http://43.205.116.221:3000/downloads/getDownloadLinks",{headers:{'Authorization':token}});
		for(let i=0;i<downloads.data.downloadLinks.length;i++){
			showDownloads(downloads.data.downloadLinks[i]);
		}
		const res=await axios.get(`http://43.205.116.221:3000/expense/getTotalExpenses`,{headers:{'Authorization':token}});
...
function addExpense(e){
	...
	axios.post("http://43.205.116.221:3000/expense/addExpense",expObject,authHeader).then((result) => {
...
function display(obj){
	...
	function deleteExpense(e){ 	
		...
		axios.delete("http://43.205.116.221:3000/expense/deleteExpense/"+id,authHeader).then((result) => {
			alert("please refresh the page");
...
async function buypremium(e){
	const response=await axios.get('http://43.205.116.221:3000/purchase/premiummembership',{headers:{"Authorization":token}});
	console.log(response);
	var options={
		"key":response.data.key_id,
		"order_id":response.data.order.id,
		"handler":async function(result){
			await axios.post('http://43.205.116.221:3000/purchase/updatetransactionstatus',{
				status:'success',
	...
	const rzp1=new Razorpay(options);
		...
		rzp1.on('payment.failed',function(result){
			axios.post('http://43.205.116.221:3000/purchase/updatetransactionstatus',{
				status:'failed',
				order_id:options.order_id,
...
function leaderBoard(){
	axios.get('http://43.205.116.221:3000/premium/leaderboard',{headers:{"Authorization":token}}).then((result) => {
...
function download(){
		axios.get('http://43.205.116.221:3000/premium/download', { headers: {"Authorization" : token} })
		.then((response) => {
